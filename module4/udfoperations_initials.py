...
def initialsUDF(firstName: str, lastName: str) -> str:
...
try:
    ...
    spark.udf.register("initialsUDF", initialsUDF, StringType())

    peopleDF = spark.createDataFrame(people, ["id", "firstName", "lastName", "age"])
    peopleWithInitials = peopleDF.withColumn("initials", call_udf("initialsUDF", col("firstName"), col("lastName")))
    peopleWithInitials.show(truncate = False)
