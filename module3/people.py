...
try:
    ...
    print(f"Number of people: {peopleDF.count()}")

    # peopleWithFullNameDF = peopleDF.withColumn("fullName", concat(col("firstName"), lit(" "), col("lastName")))\
    #     .drop("firstName", "lastName")
    peopleWithFullNameDF = peopleDF.withColumn("fullName", concat_ws(" ", col("firstName"), col("lastName")))\
        .drop("firstName", "lastName")
    peopleWithFullNameDF.show(truncate = False)
    peopleWithFullNameDF.printSchema()

    adultsDF = peopleWithFullNameDF.filter(col("age") >= 18)
